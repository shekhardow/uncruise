...
class ShipController extends Controller
{
    ...
    public function shipForm($ship_id=null) {
        ...
        if(empty($ship_id)){
            ...
            $data['ship_detail'] = $this->ship_model->getCruiseDetail($ship_id);
            $data['ship_images'] = $this->ship_model->getCruiseImages($ship_id);
        }
    ...
    public function addShip(Request $request){
        ...
        $result = $this->ship_model->addShip($requestdata, $thumbnail_image);
        if($result){
            if(!empty($size)){
                $sizetemp = [];
                foreach($size as $s){
            ...
            if(!empty($other_images)){
                foreach($other_images as $value){
                    ...
                    $this->ship_model->insertCruiseImages($data);
                }
            }
    ...
    public function updateShip(Request $request, $ship_id){
        ...
        $destination_detail = $this->ship_model->getCruiseDetail($ship_id);
        $other_images = $this->ship_model->getCruiseImages($ship_id);
        ...
        $result = $this->ship_model->updateShip($requestdata, $thumbnail_image, $ship_id);
        // dd($result); die;
        if($result){
