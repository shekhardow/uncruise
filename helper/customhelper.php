   ...
   // For Id Decryption
    ...
    function userAuthentication(Request $request)
    {
        ...
        if ($user->is_verified === 'no') {
            return response()->json(['result' => -2, 'msg' => 'Please verify yourself. We have resent the verification link to your email. Please check your mail.'], 402);
        }
    
        if ($user->status === 'Deleted') {
            return response()->json(['result' => -2, 'msg' => 'Your account has been deleted.'], 402);
        }
    
        if ($user->status === 'Disable') {
            return response()->json(['result' => -2, 'msg' => 'Your account is disabled.'], 402);
        }
    
        if ($user->status === 'Blocked') {
            return response()->json(['result' => -2, 'msg' => 'Your account is blocked.'], 402);
        }
    
        if ($user->status === 'Inactive') {
            return response()->json(['result' => -2, 'msg' => 'Your account has been inactive by admin.'], 402);
        }
