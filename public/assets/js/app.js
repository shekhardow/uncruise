...
(function ($) {
  ...
  $(".multiFilePreview input").on("change", function () {
    $(".multiFilePreview #file_preview_profile_pic").empty(); // clear any existing previews
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      reader.onload = function (event) {
        ...
        $(".multiFilePreview #file_preview_profile_pic").append(img);
      };
      reader.readAsDataURL(file);
    }
  });
  ...
  // For Favicon
  $(".multiFilePreview_favicon input").on("change", function () {
    $(".multiFilePreview_favicon #file_preview_favicon").empty(); // clear any existing previews
    var files = $(this)[0].files;
    var count = files.length;
    $(".multiFilePreview_favicon #placeholder").text(count + " file(s) selected");
    for (var i = 0; i < files.length; i++) {
      var file = files[i];
      var reader = new FileReader();
      reader.onload = function (event) {
        var img = $("<img>").attr("src", event.target.result);
        $(".multiFilePreview_favicon #file_preview_favicon").append(img);
      };
      reader.readAsDataURL(file);
    }
  });
  ...
  // For Logo
  $(".multiFilePreview_logo input").on("change", function () {
    $(".multiFilePreview_logo #file_preview_logo").empty(); // clear any existing previews
    var files = $(this)[0].files;
    var count = files.length;
    $(".multiFilePreview_logo #placeholder").text(count + " file(s) selected");
    for (var i = 0; i < files.length; i++) {
      var file = files[i];
      var reader = new FileReader();
      reader.onload = function (event) {
        var img = $("<img>").attr("src", event.target.result);
        $(".multiFilePreview_logo #file_preview_logo").append(img);
      };
      reader.readAsDataURL(file);
    }
  });
